...
function SortableNoteItem({ note, onSelect, onDelete }) {
  ...
  return (
    <div
      ...
      className="bg-kb-card border border-kb-border rounded-lg p-4 cursor-grab active:cursor-grabbing hover:border-kb-text-secondary transition-all duration-200 group select-none"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2 shrink-0">
          <button
            className="p-0.5 text-kb-text-secondary hover:text-kb-text rounded shrink-0 touch-none cursor-grab active:cursor-grabbing"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            {note.icon && <span className="text-base shrink-0">{note.icon}</span>}
            {note.priority && (
              <span
